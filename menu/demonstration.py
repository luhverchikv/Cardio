...
async def show_demo_sequence(call: CallbackQuery, bot: Bot):
    ...
    await bot.send_photo(
        photo=photo1,
        chat_id=chat_id,
        caption="🎙 Ввод занимает меньше 5 секунд. \n\nПросто нажмите и удерживайте значок микрофона и скажите, например: \n\n«Артериальное давление сто двадцать на семьдесят, пульс шестьдесят пять» \n\nБот всё распознает автоматически."
    )
    ...
    await bot.send_photo(
        photo=photo2,
        chat_id=chat_id,
        caption="📊 После нескольких измерений вы увидите наглядный график. \n\nЭто помогает:\n • заметить тенденции\n • понять, работает ли терапия\n • показать данные врачу \nРегулярность — это уже забота о себе"
    )
    await asyncio.sleep(5)
